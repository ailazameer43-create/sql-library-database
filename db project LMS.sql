...
-- SECTION 3: ALTER TABLE examples
...
-- SECTION 5: BASIC SELECT QUERIES
...
-- LEFT JOIN: all books, even if they have no category
select b.title, c.category_name
from Books b
left join Categories c on b.category_id = c.category_id

-- RIGHT JOIN: all categories, even if no book belongs to them
select b.title, c.category_name
from Books b
right join Categories c on b.category_id = c.category_id

-- FULL OUTER JOIN: all books and all categories
select b.title, c.category_name
from Books b
full outer join Categories c on b.category_id = c.category_id

-- INNER JOIN across 3 tables: borrow records with member and book info
...
-- SELF JOIN example: members who joined the same year as member 1
...
-- CROSS JOIN: all combinations of members and categories (small demo)
...
-- SP 1: borrow a book (inserts a new borrow record)
